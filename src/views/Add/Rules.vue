<template>
  <div class="container">
    ...
    <div class="form-add animate__animated animate__flash">
      <p class="main-header">اضافة حكمة</p>
      <form>
        ...
        <button type="button" class="btn btn-primary btn-lg btn-block" @click="submit">اضافة الحكمة</button>
      </form>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    submit() {
      this.$Progress.start();
      this.$http
        .post("https://vuegalal.firebaseio.com/Rules.json", this.info)
        .then(
          response => {
